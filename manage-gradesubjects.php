...
if (strlen($_SESSION['cpmsaid']==0)) {
  header('location:logout.php');
    ...
    if(isset($_POST['submit']))
            {
  $grades=$_POST['grades'];
  $subjectname=implode(',',$_POST['subjectname']);


      $dbh->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
      $sql="INSERT INTO gradesubjects (subjectname,grades) VALUES('$subjectname','$grades')";
      ...
      $mess="Record created...";
 ...
 else if(isset($_POST['update_submit']))
        {
          ...
          $dbh->query("UPDATE gradesubjects SET subjectname='$subjectname',grades='$grades' WHERE id=$id") or die($dbh->error);
          $dbh->exec($sql);
          $messagestate='added';
          $mess="Record updated...";
        }
...
else
{
  echo '<div class="popup" id="popup" style="background: rgb(206, 69, 133);"><i class="fa  fa-times"></i>&nbsp;&nbsp;'; 
    ...
    <h1 class="page-header">Manage GradeSubjects:</h1></td>
                ...
                <form method="POST" enctype="multipart/form-data"  action="manage-gradesubjects.php">
                ...
                <input type="hidden" name="id" value="<?php echo $row->id ?>">               
                  ...
                  <tr >
                      <td colspan="3"> 
        <?php
            $smt=$dbh->prepare('SELECT subjectname from subjects order by subjectname asc');
            $smt->execute();
            $data=$smt->fetchAll();
        ?>  
        <Label for="grades"> Check Subjects for the Grade</Label>  
         <div>
              <?php foreach ($data as $row):?> 
                 
                <label class="btn btn-primary text-center" >
                  <input type="checkbox" name="subjectname[]" value="<?=$row["subjectname"]?>"> <?=$row["subjectname"]?>  
                </label>      
              ...
              <?php endforeach ?>
                    ...
                    <tr >
                      <td style="width: 10%;">
                        <label for="subjectname">Grade Name:</label>
                      </td>
        ...
        ?>
          ...
          <select name="grades"  value="<?php echo $gradename; ?>" class="form-control" required="required" > 
                               ...
                               <br>
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example" style="font-family:'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif;border-style:double;">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
$sql="SELECT * FROM gardesubjects order by id desc" ;
