...
$payment_stmt = $dbh->prepare("
    ...
    GROUP BY receiptno
");
$payment_stmt->execute([':receiptno' => $ereceiptno]);
$payment_details = $payment_stmt->fetch(PDO::FETCH_OBJ);

if (!$payment_details) {
    $_SESSION['messagestate'] = 'deleted';
    $_SESSION['mess'] = "Receipt not found";
    header('location:manage-feepaymentbulk.php');
    exit;
}
...
// Fetch students already in this receipt
$receipt_students_stmt = $dbh->prepare("
    SELECT 
        fp.studentadmno,
        s.studentname,
        ce.gradefullname,
        SUM(fp.cash) AS total_paid,
        GROUP_CONCAT(CONCAT(fpv.votehead_id, ':', fpv.amount) SEPARATOR '|') AS voteheads
    FROM feepayments fp
    ...
    LEFT JOIN feepayment_voteheads fpv ON fp.id = fpv.payment_id
    WHERE fp.receiptno = :receiptno
    GROUP BY fp.studentadmno
");
$receipt_students_stmt->execute([':receiptno' => $ereceiptno]);
$receipt_students = $receipt_students_stmt->fetchAll(PDO::FETCH_OBJ);

// Get all fee voteheads
...
if (isset($_POST['receivepay_submit'])) {
    try {
        ...
        // First delete existing records for this receipt
        $delete_stmt = $dbh->prepare("DELETE FROM feepayments WHERE receiptno = :receiptno");
        $delete_stmt->execute([':receiptno' => $ereceiptno]);
        ...
        $paymentStmt = $dbh->prepare("
            ...
            (studentadmno, receiptno, cash, bank, reference, bankpaymentdate, paymentdate, details, academicyear, cashier, receiptcode, payer,entrydate,paymenttype)
            VALUES 
            (:studentadmno, :receiptno, :cash, :bank, :reference, :bankpaymentdate, :paymentdate, :details, :academicyear, :cashier, :receiptcode, :payer, :entrydate,:paymenttype)
        ");
        ...
        $_SESSION['mess'] = "Successfully updated bulk payment for $processedCount students (Total: Ksh ".number_format($grandTotal, 2).")";
...
<head>
    ...
    <title>Kipmetz-SMS | Edit Bulk Fee Payment</title>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <input type="text" class="form-control" name="receiptno" id="receiptno" required 
                                                value="<?= htmlspecialchars($payment_details->receiptno) ?>" 
                                        ...
                                        <td>
                                            ...
                                            <input type="date" class="form-control" name="paymentdate" id="paymentdate" 
                                                value="<?= htmlspecialchars($payment_details->paymentdate) ?>" required>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <input type="date" class="form-control" name="bankpaymentdate" id="bankpaymentdate" 
                                                value="<?= htmlspecialchars($payment_details->bankpaymentdate) ?>" required>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <select name="bank" id="bank" class="form-control" required
                                                ...
                                                foreach ($smt->fetchAll(PDO::FETCH_ASSOC) as $rw): ?>
                                                    <option value="<?= htmlspecialchars($rw['bankname']) ?>" <?= $rw['bankname'] == $payment_details->bank ? 'selected' : '' ?>>
                                                        <?= htmlspecialchars($rw['bankname']) ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <input type="text" class="form-control" name="academicyear" id="academicyear" 
                                                value="<?= htmlspecialchars($payment_details->academicyear) ?>" required>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <input type="text" class="form-control" name="reference" id="reference" 
                                                value="<?= isset($payment_details->reference) ? htmlspecialchars($payment_details->reference) : '' ?>" placeholder="Reference (optional)">
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="2">
                                            ...
                                            <input type="text" class="form-control" name="payer" id="payer" 
                                                value="<?= htmlspecialchars($payment_details->payer) ?>"
                                        ...
                                        <td colspan="3">
                                            ...
                                            <input type="text" class="form-control" name="details" id="details" 
                                                value="<?= htmlspecialchars($payment_details->details) ?>"
                                     ...
                                     <!-- Amount -->
                                        <td>
                                            ...
                                            <input type="number" class="form-control" name="cash" id="cash" 
                                                value="<?= 
                                                    // Calculate the sum of all cash amounts for this receipt
                                                    array_reduce($receipt_students, function($carry, $student) {
                                                        return $carry + $student->total_paid;
                                                    }, 0) 
                                                ?>" 
                                    ...
                                    <div class="panel panel-info">                                                      
                                        <div class="panel-body p-3">
                                            ...
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-hover" id="studentsTable">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $rowNum = 1;
                                                        foreach ($receipt_students as $student): 
                                                            // Parse votehead amounts for this student
                                                            $voteheadAmounts = [];
                                                            if (!empty($student->voteheads)) {
                                                                $pairs = explode('|', $student->voteheads);
                                                                foreach ($pairs as $pair) {
                                                                    if (strpos($pair, ':') !== false) {
                                                                        list($vhId, $amount) = explode(':', $pair);
                                                                        $voteheadAmounts[$vhId] = $amount;
                                                                    }
                                                                }
                                                            }
                                                        ?>
                                                        <tr data-admno="<?= htmlspecialchars($student->studentadmno) ?>">
                                                            <td><?= $rowNum++ ?></td>
                                                            <td><?= htmlspecialchars($student->studentname) ?></td>
                                                            <td><?= htmlspecialchars($student->studentadmno) ?></td>
                                                            <td class="grade-cell">
                                                                <span style="color:<?= empty($student->gradefullname) ? 'red' : 'blue' ?>; font-size:14px;">
                                                                    <?= htmlspecialchars($student->gradefullname ?? 'N/A') ?>
                                                                </span>
                                                            </td>
                                                            <?php foreach ($fee_voteheads as $vh): 
                                                                $amount = $voteheadAmounts[$vh->id] ?? 0;
                                                            ?>
                                                            <td>
                                                                <input type="number" 
                                                                       class="form-control student-votehead" 
                                                                       name="student_voteheads[<?= htmlspecialchars($student->studentadmno) ?>][<?= $vh->id ?>]" 
                                                                       step="1" 
                                                                       min="0" 
                                                            ...
                                                            <?php endforeach; ?>
                                                            <td class="total-cell"><?= number_format($student->total_paid, 2) ?></td>
                                                            <td>
                                            ...
                                            <input type="hidden" name="entrydate" id="entrydate" 
                                                value="<?= htmlspecialchars($payment_details->entrydate ?? '') ?>">
                                     ...
                                     <div class="col-md-6 d-flex align-items-center justify-content-center">
                                        <button type="submit" name="receivepay_submit" id="submitBtn" class="btn" disabled>
                                            <i class="fa fa-check-circle me-2"></i> UPDATE BULK FEE-PAYMENT
                                        </button>
                                    </div>
                                </div>
                            </form>
...
$(document).ready(function() {
    // Array to store selected students - initialize with existing students
    let selectedStudents = [
        <?php 
        foreach($receipt_students as $student) {
            echo "'".htmlspecialchars($student->studentadmno, ENT_QUOTES)."',";
        }
        ?>
    ].filter(Boolean); // Remove any empty values
    
    // Update hidden field with initial students
    $('#selectedStudents').val(selectedStudents.join(','));
    
    // Enable submit button initially if we have students
    ...
    function calculateTotalAllocation() {
        ...
        const difference = total - cashAmount;
