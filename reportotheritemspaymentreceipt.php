...
if (isset($_GET['id'])) {
    ...
    $updateSql = "UPDATE otheritemspayments SET printed = TRUE, print_date = NOW() WHERE id = :id";
...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $otheritemspaymentsql = "
            SELECT fv.*, v.otherpayitemname 
            FROM otheritemspayments_breakdown fv
            JOIN otherpayitems v ON fv.item_id = v.id
            WHERE fv.payment_id = :id
        ";
        $otheritemsquery = $dbh->prepare($otheritemspaymentsql);
        $otheritemsquery->bindParam(':id', $paymentId, PDO::PARAM_INT);
        $otheritemsquery->execute();
        $otheritemspayments = $otheritemsquery->fetchAll(PDO::FETCH_OBJ);
        ...
        $gradeQuery->bindParam(':year', $payment->financialyear, PDO::PARAM_STR);
        $gradeQuery->execute();
...
$hasTransport = $transportMatch ? true : false;
    ...
    /* otheritems Table */
    .otherpayitemname-table {
        width: 100%;
        border-collapse: collapse;
        margin: 2mm 0;
        font-size: 10px;
    }
    .otherpayitemname-table th,
    .otherpayitemname-table td {
        padding: 1mm;
        ...
        .no-print {
            ...
            <body>
                <div class="container">
                    <div class="header">
                        ...
                        <div class="details-container">
                            ...
                            <div class="payment-info">
                                ...
                                paymentmethod: <strong><?php echo htmlentities($school->paymentmethodname ?? 'Family paymentmethod'); ?></strong> | 
                    ...
                    <div class="receipt-title">OFFICIAL OTHER ITEMS PAYMENT RECEIPT</div>
                    <table class="details-table">
                        ...
                        <tr>
                            <td class="label">Latest Grade:</td>
                            <td><?php echo htmlentities($displayGrade ?? 'N/A'); ?></td>
                            <td class="label">Academic Year:</td>
                            <td><?php echo htmlentities($payment->financialyear ?? 'N/A'); ?></td>
                        </tr>
                        <tr>
                            <td class="label">Receive Date:</td>
                            <td><?php echo htmlentities(date('Y-m-d', strtotime($payment->entrydate)) ?? 'N/A'); ?></td>
                            <td class="label">Bank Payment Date:</td>
                            <td><?php echo htmlentities(date('Y-m-d', strtotime($payment->bankpaymentdate)) ?? 'N/A'); ?></td>
                        </tr>

                    </table>

                    <table class="otherpayitemname-table">
                        <tr>
                            <th>#</th>
                            <th>Item</th>
                        ...
                        $total = 0;
                        $counter = 1;
                        if (!empty($otheritemspayments)) {
                            foreach ($otheritemspayments as $otherpayitemname) {
                                $total += $otherpayitemname->amount;
                                ?>
                                <tr>
                                    <td><?php echo $counter++; ?>.</td>
                                    <td><?php echo htmlentities($otherpayitemname->otherpayitemname); ?></td>
                                    <td class="amount-cell"><?php echo number_format($otherpayitemname->amount, 2); ?></td>
                                </tr>
                                <?php
                            }
                        } else {
                            ?>
                            <tr>
                                <td colspan="3" style="text-align: center;">No otherpayitemname details available</td>
                            </tr>
                            <?php
                        }
                        ?>
                    ...
                    <table class="details-table">
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlentities($payment->paymentmethod ?$payment->paymentmethod : 'Cash'); ?></td>
